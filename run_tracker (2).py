...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    for i in range(1,len(img_list)):
        ...
        if success:
            ...
            segment = slic(image2, n_segments = 500, sigma = 5)
            pos_feats1 = forward_samples(model, image2*segment, pos_examples)
            neg_feats1 = forward_samples(model, image2*segment, neg_examples)
            pos_feats = 0.5*pos_feats+0.5*pos_feats*pos_feats1
            neg_feats = 0.5*neg_feats+0.5*neg_feats*neg_feats1
